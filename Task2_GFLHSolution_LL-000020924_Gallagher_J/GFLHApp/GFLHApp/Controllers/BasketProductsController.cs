...
namespace GFLHApp.Controllers // Places these MVC controller types in the application controllers namespace.
{
    ...
    public class BasketProductsController : Controller // Defines the MVC controller for adding, removing, and counting products inside customer baskets.
    {
        ...
        // ----- Ajax Basket Actions -----
        // Remove basket
        public async Task<IActionResult> RemoveAjax(int id) // Removes a basket item and returns updated totals as JSON without redirecting
        {
            // Set user ID
            var userId = User.FindFirstValue(ClaimTypes.NameIdentifier); // Gets the current signed-in user's Identity id from claims.
            if (userId == null) // Checks that a signed-in user id is available.
                return Json(new { success = false, message = "Not authorised." }); // Returns a JSON response for client-side code.
            ...
            if (basketProduct == null) // Checks whether the requested data was found.
                return Json(new { success = false, message = "Item not found." }); // Returns a JSON response for client-side code.

            var basketId = basketProduct.BasketId; // Stores the basket ID before removal for the totals recalculation.

            _context.BasketProducts.Remove(basketProduct); // Queues the entity for removal from the database.
            await _context.SaveChangesAsync(); // Persists pending database changes asynchronously.

            // Recalculate totals
            var remaining = await _context.BasketProducts // Sets remaining to the value needed by the workflow.
                .Include(bp => bp.Products) // Includes related records needed by the view or workflow.
                .Where(bp => bp.BasketId == basketId) // Filters the query to only the relevant records.
                .ToListAsync(); // Executes the query asynchronously and materializes the list.

            decimal subtotal = remaining.Sum(bp => bp.Products.ItemPrice * bp.ProductQuantity); // Calculates the updated subtotal after removal.

            // Loyalty system
            var orderCount = await _context.Orders.CountAsync(o => o.UserId == userId); // Counts matching records for display or validation.

            // Health bundle
            var productNames = remaining.Select(x => x.Products.ItemName.ToLower()).ToList(); // Projects the query results into the shape needed next.
            bool hasHealthBundle = productNames.Contains("broccoli") && productNames.Contains("carrot") && productNames.Contains("apple"); // Checks whether the health bundle discount applies.
            ...
            if (orderCount % 5 == 4) // Checks the condition that controls the next action.
                discount = subtotal * 0.15m; // Calculates the loyalty discount value.
            else if (hasHealthBundle) // Checks the next fallback condition in the workflow.
                discount = subtotal * 0.10m; // Calculates the health bundle discount value.

            decimal total = subtotal - discount; // Calculates the total value used for totals.

            return Json(new // Returns a JSON response for client-side code.
            {
                success = true,
                isEmpty = !remaining.Any(),
                itemCount = remaining.Sum(bp => bp.ProductQuantity),
                subtotal = subtotal.ToString("0.00"),
                discount = discount.ToString("0.00"),
                total = total.ToString("0.00"),
                hasDiscount = discount > 0,
                hasHealthBundle,
                discountLabel = hasHealthBundle ? "Health bundle" : "Loyalty reward",
                savedMessage = hasHealthBundle ? $"You saved £{discount:0.00} with the health bundle!" : $"You saved £{discount:0.00} with your loyalty reward!"
            });
        }
