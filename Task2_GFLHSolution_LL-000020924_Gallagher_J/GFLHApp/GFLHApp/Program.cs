...
using Microsoft.EntityFrameworkCore.Infrastructure; // Imports infrastructure helpers used for running startup database checks.
...
using (var scope = app.Services.CreateScope()) // Imports (var scope = app.Services.CreateScope()) types used by this file.
{
    ...
    await context.Database.MigrateAsync(); // Applies any pending EF Core migrations before the app reads or seeds data.
    // ----- Identity configuration -----
    ...
    // ----- Seed orders -----
    await SeedData.SeedOrders(services); // Creates the default order records for the five seeded users.
}
...
app.UseAuthentication(); // Loads the signed-in user and roles before authorization checks run.
app.UseAuthorization(); // Applies authorization checks to incoming requests.

// ----- 404 handling -----
app.Use(async (context, next) => // Intercepts responses after the controller has run.
{
    await next(); // Lets the controller and all other middleware run first.
    if (context.Response.StatusCode == 404 && !context.Response.HasStarted) // Catches 404s before any response body is sent.
    {
        context.Response.Redirect("/Identity/Account/AccessDenied"); // Redirects missing records to the access denied page.
    }
});
